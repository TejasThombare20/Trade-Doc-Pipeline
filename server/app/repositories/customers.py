...
async def list_customers(conn: asyncpg.Connection, tenant_id: UUID) -> list[asyncpg.Record]:
...
async def get_customer(
    conn: asyncpg.Connection, tenant_id: UUID, customer_id: UUID
) -> asyncpg.Record | None:
...
async def get_customer_by_code(
    conn: asyncpg.Connection, tenant_id: UUID, code: str
) -> asyncpg.Record | None:
    return await conn.fetchrow(
